...
use crate::embeddings::{EmbeddingsManager, EmbeddingsConfig};
...
impl EmbeddingModelBridge {
    ...
    pub fn new(manager: Option<EmbeddingsManager>) -> Self {
        Self { manager }
    }
    ...
    pub fn is_enabled(&self) -> bool {
        self.manager.as_ref().map(|m| m.is_enabled()).unwrap_or(false)
    }
}
