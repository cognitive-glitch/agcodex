...
use super::registry::{ToolError, ToolOutput};
...
use super::glob::{GlobTool, FileType};
...
pub fn adapt_think_tool(input: Value) -> Result<ToolOutput, ToolError> {
    ...
    let result = if language.is_some() || context.is_some() {
        ...
        // Use basic thinking
        let basic_result = ThinkTool::think(problem)
            .map_err(|e| ToolError::ExecutionFailed(e.to_string()))?;
...
pub fn adapt_plan_tool(input: Value) -> Result<ToolOutput, ToolError> {
    ...
    let result = tool.plan(&full_goal)
        .map_err(|e| ToolError::ExecutionFailed(e.to_string()))?;
    
    // Convert tasks to JSON format
    let tasks_json: Vec<Value> = result.tasks.iter().map(|task| {
        json!({
    ...
    let parallel_groups_json: Vec<Value> = result.parallel_groups.iter().map(|group| {
        json!(group.iter().map(|id| id.to_string()).collect::<Vec<_>>())
    }).collect();
...
pub fn adapt_glob_tool(input: Value) -> Result<ToolOutput, ToolError> {
    ...
    let result = tool.glob(pattern)
        .map_err(|e| ToolError::ExecutionFailed(e.to_string()))?;
    
    // Map the FileMatch results to JSON
    let files: Vec<Value> = result.result.iter().map(|m| json!({
    ...
    let duration_ms = result.metadata.completed_at
    ...
    Ok(ToolOutput::success(
        ...
        format!("Found {} files matching pattern '{}'", result.result.len(), pattern)
    ))
}
...
pub fn adapt_tree_tool(input: Value) -> Result<ToolOutput, ToolError> {
    ...
    let result = rt.block_on(async {
        ...
        let parse_result = tool.parse_file(PathBuf::from(file_path)).await
            .map_err(|e| ToolError::ExecutionFailed(e.to_string()))?;
...
pub fn adapt_bash_tool(input: Value) -> Result<ToolOutput, ToolError> {
    ...
    if !read_only_commands.contains(&first_word) {
        return Err(ToolError::InvalidInput(
            format!("Command '{}' not allowed in safe mode", first_word)
        ));
    }
