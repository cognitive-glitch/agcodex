...
use crate::subagents::InvocationProcessor;
...
impl ConversationManagerExt {
    ...
    pub fn new(_config: &Config) -> CodexResult<Self> {
        ...
        let registry =
            Arc::new(SubagentRegistry::new().map_err(|e| {
                CodexErr::InvalidConfig(format!("Failed to create registry: {}", e))
            })?);
            ...
            match processor.process_message(message).await {
                ...
                Err(e) => {
                    ...
                    if context.fail_on_agent_error {
                        Err(CodexErr::InvalidConfig(format!(
    ...
    pub const fn inner(&self) -> &ConversationManager {
        &self.inner
    }

    /// Get the subagent registry
    pub const fn registry(&self) -> &Arc<SubagentRegistry> {
        &self.registry
    }
...
pub enum InterceptResult {
    ...
    Handled { response: String, skip_llm: bool },
}
