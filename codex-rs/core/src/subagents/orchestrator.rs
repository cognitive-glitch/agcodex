...
use super::{
    AgentChain, AgentInvocation, ExecutionPlan, ExecutionStep, InvocationRequest,
    SubagentContext, SubagentError, SubagentExecution, SubagentRegistry, SubagentStatus,
};
...
impl SharedContext {
    ...
    pub async fn merge(&self, other: &SharedContext) {
        ...
        self.previous_outputs.write().await.extend(other_outputs.clone());
        
        // Merge modified files
        let other_files = other.modified_files.read().await;
        self.modified_files.write().await.extend(other_files.clone());
    ...
    async fn record_failure(&self) {
        ...
        *self.last_failure_time.lock().await = Some(SystemTime::now());
        ...
        if count >= self.threshold {
            ...
            warn!("Circuit breaker opened after {} consecutive failures", count);
        }
    }
...
impl AgentOrchestrator {
    ...
    pub async fn execute_plan(
        ...
        if self.cancelled.load(Ordering::SeqCst) {
            return Err(SubagentError::ExecutionFailed("Execution cancelled".to_string()));
        }
        ...
        let executions = match &request.execution_plan {
            ...
            ExecutionPlan::Mixed(steps) => {
                self.execute_mixed(steps.clone(), &context).await?
            }
        };
        ...
        let success = executions.iter().all(|e| e.status == SubagentStatus::Completed);
        let partial_success = executions.iter().any(|e| e.status == SubagentStatus::Completed);
    ...
    pub async fn execute_single(
        ...
        if self.config.enable_circuit_breaker {
            let breaker = self.get_or_create_circuit_breaker(&invocation.agent_name).await;
            if breaker.is_open().await {
                return Err(SubagentError::ExecutionFailed(
                    format!("Circuit breaker open for agent {}", invocation.agent_name)
                ));
            }
        }
        ...
        let _permit = self.concurrency_limiter.acquire().await
            .map_err(|e| SubagentError::ExecutionFailed(format!("Failed to acquire permit: {}", e)))?;
        ...
        if self.config.enable_circuit_breaker {
            let breaker = self.get_or_create_circuit_breaker(
                &result.as_ref().map(|e| e.agent_name.clone()).unwrap_or_default()
            ).await;
    ...
    pub async fn execute_sequential(
        ...
        for invocation in chain.agents {
            ...
            shared_context.add_modified_files(execution.modified_files.clone()).await;
    ...
    pub async fn execute_parallel(
        ...
        for invocation in invocations {
            ...
            let task = tokio::spawn(async move {
                self_clone.execute_single(invocation, &context_clone).await
            });
        ...
        if !executions.is_empty() {
            ...
            Err(SubagentError::ExecutionFailed(
                format!("All parallel executions failed: {}", errors.join(", "))
            ))
        } else {
            Err(SubagentError::ExecutionFailed("No executions completed".to_string()))
        }
    }
    ...
    async fn execute_with_retry(
        ...
        loop {
            ...
            self.send_progress(ProgressUpdate {
                ...
                message: Some(format!("Starting execution (attempt {}/{})", attempts, max_attempts)),
            ...
            match result {
                ...
                Err(_) => {
                    // Timeout
                    let error = format!("Agent {} timed out after {:?}", invocation.agent_name, self.config.agent_timeout);
                    ...
                    if attempts < max_attempts {
                        warn!("Agent {} timed out (attempt {}/{})", invocation.agent_name, attempts, max_attempts);
                        sleep(self.config.retry_backoff * attempts).await;
                        continue;
                    }
                    
                    return Err(SubagentError::Timeout { name: invocation.agent_name });
                }
            }
        }
    }
    ...
    async fn execute_agent_internal(
        ...
        let config = self.registry.get_agent(&invocation.agent_name).await?;
        
        // Check mode restrictions
        if let Some(mode_override) = &config.mode_override {
            if *mode_override != self.operating_mode {
                return Err(SubagentError::ModeRestriction {
                    mode: self.operating_mode,
                    operation: format!("Agent {} requires mode {:?}", invocation.agent_name, mode_override),
                });
            }
        }
        ...
        let agent_context = SubagentContext {
            execution_id: execution.id,
            mode: config.mode_override.unwrap_or(self.operating_mode),
            available_tools: config.tools.iter().map(|t| format!("{:?}", t)).collect(),
    ...
    fn is_retriable_error(&self, error: &SubagentError) -> bool {
        matches!(
            error,
    ...
    async fn check_memory_pressure(&self) -> Result<(), SubagentError> {
        ...
        if memory_usage_mb > self.config.memory_threshold_mb {
            Err(SubagentError::ExecutionFailed(
                format!("Memory pressure too high: {}MB > {}MB threshold",
    ...
    pub async fn execute_with_dependencies(
        ...
        for dep in dependencies {
            if !outputs.iter().any(|o| o.contains(&dep)) {
                return Err(SubagentError::ExecutionFailed(
                    format!("Dependency {} not satisfied for agent {}", dep, invocation.agent_name)
                ));
            }
        }
...
mod tests {
    ...
    async fn test_sequential_execution() {
        ...
        assert!(executions.iter().all(|e| e.status == SubagentStatus::Completed));
    }
    ...
    async fn test_context_sharing() {
        ...
        // Set and get values
        context.set("key1".to_string(), serde_json::json!("value1")).await;
        ...
        context.set("key2".to_string(), serde_json::json!("value2")).await;
    ...
    async fn test_cancellation() {
        ...
        assert!(matches!(result.unwrap_err(), SubagentError::ExecutionFailed(_)));
    }
}