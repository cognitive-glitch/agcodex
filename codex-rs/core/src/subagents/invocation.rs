...
impl InvocationParser {
    ...
    pub fn new() -> Self {
        let agent_pattern = Regex::new(r"@([a-zA-Z0-9_-]+)(?:\s+([^@→+]*?))?(?:\s*[@→+]|\s*$)")
    ...
    fn parse_parameters(&self, param_str: &str) -> Result<HashMap<String, String>, super::SubagentError> {
            ...
            match ch {
                ...
                '"' if in_quotes => {
                    ...
                    if !current_key.is_empty() {
                        ...
                        parameters.insert(format!("arg{}", positional_index), current_value.clone());
                        positional_index += 1;
                    }
                    current_key.clear();
                    current_value.clear();
    ...
    fn build_execution_plan(
        ...
        if invocations.len() == 1 {
            return Ok(ExecutionPlan::Single(invocations.into_iter().next().unwrap()));
        }
        ...
        // Complex mixed execution (both → and +)
    ...
    pub fn validate_execution_plan(&self, plan: &ExecutionPlan) -> Result<(), super::SubagentError> {
        let agents: Vec<&String> = match plan {
            ExecutionPlan::Single(_) => return Ok(()), // No cycles possible
            ExecutionPlan::Sequential(chain) => chain.agents.iter().map(|a| &a.agent_name).collect(),
            ...
            ExecutionPlan::Mixed(steps) => {
                ...
                steps
                    ...
                    .flat_map(|step| match step {
                        ...
                        ExecutionStep::Parallel(invs) => invs.iter().map(|inv| &inv.agent_name).collect(),
...
mod tests {
    ...
    fn test_sequential_chain_parsing() {
        let parser = InvocationParser::new();
        let result = parser.parse("@refactorer fix → @test-writer add tests").unwrap().unwrap();
    ...
    fn test_parallel_execution_parsing() {
        let parser = InvocationParser::new();
        let result = parser.parse("@performance analyze + @security audit").unwrap().unwrap();
    ...
    fn test_parameter_parsing() {
        ...
        // Test quoted values
        let params = parser.parse_parameters(r#"message="fix this bug" priority=1"#).unwrap();
    ...
    fn test_context_extraction() {
        ...
        assert_eq!(result.context, "Please this file and then . Make sure everything works.");
    }
    ...
    fn test_circular_dependency_detection() {
        ...
        assert!(matches!(result.unwrap_err(), super::super::SubagentError::CircularDependency { .. }));
    }
}