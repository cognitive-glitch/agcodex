...
use crate::code_tools::ast_agent_tools::{
    ASTAgentTools, AgentToolOp, AgentToolResult, PatternType, SearchScope,
    DocumentationTarget, ImprovementFocus, Location
};
use crate::subagents::{SubagentContext, SubagentError, SubagentResult};
...
impl CodeReviewerAgent {
    ...
    async fn analyze_functions(&self, ast_tools: &mut ASTAgentTools, file: &PathBuf) -> SubagentResult<Vec<Finding>> {
        ...
        if let AgentToolResult::Functions(functions) = result {
            for func in functions {
                // Check complexity
                if let Ok(AgentToolResult::Complexity(complexity)) = ast_tools.execute(
                    AgentToolOp::CalculateComplexity {
                    ...
                    if complexity.cyclomatic_complexity > 10 {
                        findings.push(Finding {
                            ...
                            suggestion: Some("Break down into smaller functions or reduce conditional complexity".to_string()),
                            metadata: HashMap::from([
                                ("complexity".to_string(), serde_json::Value::Number(
                                    serde_json::Number::from(complexity.cyclomatic_complexity)
                                )),
                            ]),
                        });
                    }
                ...
                if line_count > 50 {
                    findings.push(Finding {
                        ...
                        metadata: HashMap::from([
                            ("line_count".to_string(), serde_json::Value::Number(
                ...
                if func.parameters.len() > 5 {
                    findings.push(Finding {
                        ...
                        suggestion: Some("Group related parameters into a struct or use builder pattern".to_string()),
                        metadata: HashMap::from([
                            ("parameter_count".to_string(), serde_json::Value::Number(
                                serde_json::Number::from(func.parameters.len())
                            )),
                        ]),
                    });
                }
    ...
    async fn detect_security_issues(&self, ast_tools: &mut ASTAgentTools) -> SubagentResult<Vec<Finding>> {
        let mut findings = Vec::new();
        ...
        if let AgentToolResult::DeadCode(dead_items) = result {
            for item in dead_items {
                findings.push(Finding {
                    ...
                    metadata: HashMap::from([
                        ("item_type".to_string(), serde_json::Value::String(format!("{:?}", item.kind))),
                    ]),
                });
            }
        }
...
impl Subagent for CodeReviewerAgent {
    ...
    fn supports_file_type(&self, file_path: &std::path::Path) -> bool {
        matches!(
            ...
            Some("go") | Some("java") | Some("cpp") | Some("c") | Some("cs")
        )
    }
...
impl CodeReviewerAgent {
    fn get_files_to_analyze(&self, working_dir: &PathBuf) -> SubagentResult<Vec<PathBuf>> {
        ...
        for entry in WalkDir::new(working_dir)
            ...
            .max_depth(10) 
        {
            let entry = entry.map_err(|e| SubagentError::Io(std::io::Error::new(
    ...
    fn generate_summary(&self, findings: &[Finding]) -> String {
        ...
        format!(
            ...
            findings.len(), critical_count, high_count, medium_count, low_count
        )
    }
    
    fn calculate_metrics(&self, findings: &[Finding], analyzed_files: &[PathBuf]) -> HashMap<String, serde_json::Value> {
        let mut metrics = HashMap::new();
        
        metrics.insert("total_findings".to_string(), serde_json::Value::Number(
            serde_json::Number::from(findings.len())
        ));
        metrics.insert("files_analyzed".to_string(), serde_json::Value::Number(
            serde_json::Number::from(analyzed_files.len())
        ));
        ...
        for severity in [Severity::Critical, Severity::High, Severity::Medium, Severity::Low, Severity::Info] {
            ...
            metrics.insert(key, serde_json::Value::Number(serde_json::Number::from(count)));
        }
        ...
        for (category, count) in categories {
            metrics.insert(format!("{}_issues", category), serde_json::Value::Number(
        ...
        if let AgentToolResult::Duplications(duplications) = result {
            for dup_group in duplications {
                findings.push(Finding {
                    ...
                    severity: if dup_group.locations.len() > 3 { Severity::High } else { Severity::Medium },
                    title: "Code duplication detected".to_string(),
                    description: format!("Found {} instances of duplicated code", dup_group.locations.len()),
                    location: dup_group.locations.first().cloned(),
                    suggestion: Some("Extract common functionality into a shared function or method".to_string()),
                    metadata: HashMap::from([
                        ("duplication_count".to_string(), serde_json::Value::Number(
                            serde_json::Number::from(dup_group.locations.len())
                        )),
                        ("similarity_score".to_string(), serde_json::Value::Number(
                            serde_json::Number::from_f64(dup_group.similarity as f64).unwrap()
                        )),
                    ]),
                });
            }
        ...
        if let Some(entry_point) = context.parameters.get("entry_point") {
            ...
            if let AgentToolResult::CallGraph(call_graph) = result {
                ...
                for (_name, node) in call_graph.nodes {
                    ...
                    let has_callers = call_graph.edges.iter().any(|edge| edge.callee == node.function_name);
                    if !has_callers {
                        findings.push(Finding {
                            category: "unreachable".to_string(),
                            severity: Severity::Medium,
                            title: format!("Potentially unreachable function: {}", node.function_name),
                            description: "Function with no callers detected".to_string(),
                            location: Some(node.location),
                            suggestion: Some("Verify if this function is used externally or can be removed".to_string()),
                            metadata: HashMap::new(),
                        });
                    }
                }
...
impl Subagent for TestWriterAgent {
    ...
    async fn execute(
        ...
        for file in &files {
            ...
            if let AgentToolResult::Functions(functions) = result {
                for func in functions {
                    ...
                    let is_test_function = func.name.contains("test") || func.name.starts_with("test_");
                    if !is_test_function && func.is_exported {
                        findings.push(Finding {
                            category: "testing".to_string(),
                            severity: Severity::Medium,
                            title: format!("Missing tests for function: {}", func.name),
                            description: format!("Public function '{}' lacks test coverage", func.name),
                            location: Some(Location {
                            file: file.clone(),
                            line: func.start_line,
                            column: 0,
                            ...
                            suggestion: Some("Add unit tests to verify function behavior".to_string()),
                            metadata: HashMap::from([
                                ("function_name".to_string(), serde_json::Value::String(func.name)),
                                ("parameter_count".to_string(), serde_json::Value::Number(
                                    serde_json::Number::from(func.parameters.len())
                                )),
                            ]),
        ...
        Ok(AgentResult {
            ...
            modified_files: vec![],
            execution_time: start_time.elapsed().unwrap_or_default(),
            summary: format!("Test analysis completed. Found {} functions needing tests.", findings_count),
            metrics: HashMap::new(),
        })
    }
    
    fn capabilities(&self) -> Vec<String> {
        vec!["extract_functions".to_string(), "generate_tests".to_string()]
    }
...
impl Subagent for PerformanceAgent {
    ...
    async fn execute(
        ...
        if target_file.exists() {
            ...
            if let AgentToolResult::Improvements(improvements) = result {
                for improvement in improvements {
                    let severity = match improvement.impact {
                        // Note: Assuming ImpactLevel is an enum that needs to be matched appropriately
                        // This is a placeholder - adjust based on actual ImpactLevel definition
                        _ => Severity::Medium,
                    };
                    ...
                    findings.push(Finding {
                        ...
                        metadata: HashMap::from([
                            ("category".to_string(), serde_json::Value::String(format!("{:?}", improvement.category))),
                            ("impact".to_string(), serde_json::Value::String(format!("{:?}", improvement.impact))),
                        ]),
                    });
                }
            }
        ...
        Ok(AgentResult {
            ...
            modified_files: vec![],
            execution_time: start_time.elapsed().unwrap_or_default(),
            summary: format!("Performance analysis completed. Found {} optimization opportunities.", findings_count),
            metrics: HashMap::new(),
        })
    }
...
impl Subagent for SecurityAgent {
    ...
    async fn execute(
            ...
            if let AgentToolResult::Patterns(patterns) = result {
                for detected_pattern in patterns {
                    let (severity, description, suggestion) = match pattern {
                        ...
                        _ => (Severity::Medium, "Security issue detected", "Review and address security concern"),
                    };
                    ...
                    findings.push(Finding {
                        ...
                        metadata: HashMap::from([
                            ("vulnerability_type".to_string(), serde_json::Value::String(pattern.to_string())),
                        ]),
                    });
                }
            }
        ...
        Ok(AgentResult {
            ...
            modified_files: vec![],
            execution_time: start_time.elapsed().unwrap_or_default(),
            summary: format!("Security analysis completed. Found {} potential vulnerabilities.", findings_count),
            metrics: HashMap::new(),
        })
    }
    ...
    fn supports_file_type(&self, file_path: &std::path::Path) -> bool {
        matches!(
            file_path.extension().and_then(|ext| ext.to_str()),
            Some("rs") | Some("py") | Some("js") | Some("ts") | Some("jsx") | Some("tsx") | 
            Some("go") | Some("java") | Some("cpp") | Some("c") | Some("cs") | Some("php")
        )
    }
}
...
impl Subagent for DocsAgent {
        ...
        if let Some(target_file) = context.parameters.get("target") {
            ...
            if let AgentToolResult::Documentation(doc_content) = result {
                findings.push(Finding {
                    ...
                    suggestion: None,
                    metadata: HashMap::from([
                        ("doc_length".to_string(), serde_json::Value::Number(
                            serde_json::Number::from(doc_content.len())
                        )),
                    ]),
                });
            }
    ...
    fn capabilities(&self) -> Vec<String> {
        vec!["generate_documentation".to_string(), "extract_functions".to_string()]
    }
...
impl AgentRegistry {
    pub fn new() -> Self {
        ...
        registry.register(Box::new(CodeReviewerAgent::new()));
        registry.register(Box::new(RefactorerAgent::new()));
        registry.register(Box::new(DebuggerAgent::new()));
        registry.register(Box::new(TestWriterAgent::new()));
        registry.register(Box::new(PerformanceAgent::new()));
        registry.register(Box::new(SecurityAgent::new()));
        registry.register(Box::new(DocsAgent::new()));
    ...
    pub fn get_agents_for_file(&self, file_path: &std::path::Path) -> Vec<&str> {
        self.agents
            .iter()
