...
//!
//! The history is stored at `~/.agcodex/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.agcodex`.
...
pub(crate) async fn append_entry(text: &str, session_id: &Uuid, config: &Config) -> Result<()> {
    ...
    // Resolve `~/.agcodex/history.jsonl` and ensure the parent directory exists.
