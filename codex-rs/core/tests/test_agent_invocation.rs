...
use agcodex_core::{ConversationManagerExt, MessageContext, InterceptResult};
...
async fn test_multiple_agent_invocation() {
    ...
    let result = manager
        .intercept_message(message, context.clone())
        .await;
...
async fn test_result_merging() {
    ...
    if let Ok(InterceptResult::Modified { message: merged, .. }) = result {
        // In a real scenario, this would contain merged agent outputs
        assert!(merged.contains("Agent Analysis") || merged.len() > 0);
    }
