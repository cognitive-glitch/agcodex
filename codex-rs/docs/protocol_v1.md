...
These are entities exit on the agcodex backend. The intent of this section is to establish vocabulary and construct a shared mental model for the `AGCodex` core system.

0. `Model`
   - In our case, this is the Responses REST API
1. `AGCodex`
   - The core engine of agcodex
...
2. `Session`
   - The `AGCodex`'s current configuration and state
   - `AGCodex` starts with no `Session`, and it is initialized by `Op::ConfigureSession`, which should be the first message sent by the UI.
...
3. `Task`
   - A `Task` is `AGCodex` executing work in response to user input.
...
4. `Turn`
   - One cycle of iteration in a `Task`, consists of:
     ...
     - `AGCodex` then executes command(s), applies patch(es), and outputs message(s) returned by the `Model`
...
The term "UI" is used to refer to the application driving `AGCodex`. This may be the CLI / TUI chat-like interface that users operate, or it may be a GUI interface like a VSCode extension. The UI is external to `AGCodex`, as `AGCodex` is intended to be operated by arbitrary UI implementations.
...
Since only 1 `Task` can be run at a time, for parallel tasks it is recommended that a single `AGCodex` be run for each thread of work.
...
- `Submission`
  - These are messages sent on the `SQ` (UI -> `AGCodex`)
...
- `Event`
  - These are messages sent on the `EQ` (`AGCodex` -> UI)
...
sequenceDiagram
    ...
    box Daemon
    participant agcodex as AGCodex
    ...
    user->>agcodex: Op::ConfigureSession
    agcodex-->>session: create session
    agcodex->>user: Event::SessionConfigured
