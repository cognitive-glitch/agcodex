...
use agcodex_persistence::types::{SessionIndex, SessionMetadata, CheckpointMetadata};
...
fn create_sample_session_metadata(title: &str, mode: OperatingMode, favorite: bool) -> SessionMetadata {
    SessionMetadata {
        id: Uuid::new_v4(),
        ...
        message_count: (10..50).into_iter().nth(rand::random::<usize>() % 40).unwrap_or(15),
        turn_count: (5..25).into_iter().nth(rand::random::<usize>() % 20).unwrap_or(10),
        current_mode: mode,
        ...
        file_size: (1024..10240).into_iter().nth(rand::random::<usize>() % 9216).unwrap_or(2048) as u64,
...
fn create_sample_session_index() -> SessionIndex {
    ...
    let sessions = vec![
        create_sample_session_metadata("Refactor authentication system", OperatingMode::Build, true),
        create_sample_session_metadata("Fix CSS layout issues", OperatingMode::Review, false),
        create_sample_session_metadata("Plan new feature: real-time chat", OperatingMode::Plan, true),
        ...
        create_sample_session_metadata("Experiment with new UI library", OperatingMode::Build, false),
